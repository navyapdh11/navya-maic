...
- NAVYA-OPENCLASS is healthy at the selected `url`
- Provider keys are configured

> **Hosted mode**: If using hosted NAVYA-OPENCLASS (open.maic.chat), all
...
- Do not try to recover from auth, provider, model, or base URL errors by changing request parameters. Tell the user to fix NAVYA-OPENCLASS server-side config and retry only after they confirm.
