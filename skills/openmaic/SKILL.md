...
- Do not assume the OpenClaw agent's own model or API key will be reused by NAVYA-OPENCLASS.
- NAVYA-OPENCLASS classroom generation uses NAVYA-OPENCLASS server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only NAVYA-OPENCLASS server-side config files may control provider selection and defaults.
...
If no `accessCode` in config, ask the user how they want to use NAVYA-OPENCLASS:

1. **Use hosted NAVYA-OPENCLASS** (recommended for quick start) — Requires an access code from open.maic.chat. No local setup needed.
...
Use this when the user has not installed NAVYA-OPENCLASS yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify NAVYA-OPENCLASS

After the user has chosen a startup mode and configured keys, start NAVYA-OPENCLASS using the chosen method, then verify the service with `GET {url}/api/health`.
